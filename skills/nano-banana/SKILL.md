...
| Output directory missing | Uses `./nanobanana-output/` only; created automatically on first run |
| **No valid API key found** (MCP) | See “API key when running under MCP” below |
### API key when running under MCP

When nanobanana runs via Gemini CLI's MCP (e.g. from Cursor or another host), the extension's MCP process runs **in a separate process** and does not inherit the current run's `GEMINI_API_KEY`. Setting `NANOBANANA_API_KEY=$GEMINI_API_KEY` in the agent env is not enough if the MCP never sees it.

**Required (ops/Board):** In the **environment where the nanobanana MCP process runs**, configure one of:
...
Use the same Gemini key as in the company Secret. If Gemini CLI is started via Cursor or a local MCP server, inject the key in that MCP's config or in `gemini extensions config nanobanana` (if supported). After this, re-run the task or trigger the agent again.

**Temporary workaround:** Run `gemini` in the workspace shell so the key is inherited by the process:

```bash
cd <workspace_cwd> && GEMINI_API_KEY=your_key gemini --yolo "/icon '...' --sizes=256,512 --type=app-icon --corners=rounded"
```

Output will be under `./nanobanana-output/`.
